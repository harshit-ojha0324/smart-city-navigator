...
src/navigator/
  ...
  mcp_servers/   3 MCP servers over streamable HTTP
...
- **No adapter in the critical path.** `langchain-mcp-adapters` pins `mcp<2.0.0`,
  ...
  the server's own JSON schema, and open a session per call. The integration test
  boots the three servers as real processes and drives them through it.
