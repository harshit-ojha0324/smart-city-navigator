...
def create_app() -> Flask:
    ...
    def stream():
        ...
        thread_id = (request.args.get("thread_id") or "default").strip()
        if not question:
...
<style>
 ...
 .answer{margin-top:1rem;padding:1rem;background:#EAF0FF;border-radius:8px;font-size:1.05rem}
...
f.onsubmit=e=>{e.preventDefault();if(!q.value.trim())return;
 ...
 es.onmessage=ev=>{const d=JSON.parse(ev.data);
  ...
  steps.innerHTML+=`<span class=node>[${d.node}]</span> ${d.text||''}\\n`;};
 es.onerror=()=>es.close();};
</script></body></html>"""
